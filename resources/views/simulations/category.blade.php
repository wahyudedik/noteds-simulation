<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">{{ $category }}</h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-sm text-gray-500 dark:text-gray-400">({{ $simulations->total() }} experience)</span>
                </div>
                <div class="flex items-center gap-1">
                    ...
                    @foreach($sortOptions as $key => $label)
                        <a href="{{ route('simulations.category', ['category' => $category, 'sort' => $key]) }}"
                            class="px-3 py-1.5 text-xs font-medium rounded-lg transition {{ ($sort ?? 'popular') === $key ? 'bg-blue-600 text-white dark:bg-blue-500' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                            {{ $label }}
                        </a>
                    @endforeach
                </div>
            ...
            @else
                <div class="text-center py-16">
                    ...
                    <p class="text-gray-500 dark:text-gray-400">Belum ada experience di kategori ini.</p>
                </div>
            @endif
        </div>
    </div>
