        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div id="player-wrapper"
                    data-serve-url="{{ route('simulations.serve', ['slug' => $simulation->slug, 'path' => $simulation->entry_point ?? 'index.html']) }}"
                    data-play-url="{{ route('simulations.play', $simulation->slug) }}"
                    data-simulation-id="{{ $simulation->id }}"
                    data-comments-url="{{ route('comments.store', $simulation->slug) }}"
                    data-favorites-url="{{ route('favorites.toggle', $simulation->id) }}"
                    data-bookmarks-url="{{ route('bookmarks.toggle') }}"
                    data-share-url="{{ route('simulations.share', $simulation->id) }}"
                    data-reactions-url="{{ route('reactions.toggle') }}"
                    data-ratings-url="{{ route('ratings.store') }}"
                    data-collections-url="{{ route('collections.add-simulation') }}"
                >
                    {{-- Thumbnail / Poster --}}
                    <div id="player-poster" class="bg-black rounded-xl overflow-hidden aspect-video relative">
                        ...
                        <div id="play-overlay" class="absolute inset-0 flex items-center justify-center bg-black/30 cursor-pointer" role="button" tabindex="0" aria-label="Putar simulasi" onclick="playSimulation()" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();playSimulation();}">
