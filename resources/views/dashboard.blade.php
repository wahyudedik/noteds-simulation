        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 rounded-xl p-6">
                <div class="flex items-center gap-5">
                    ...
                    @else
                        <div class="w-16 h-16 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 flex items-center justify-center text-2xl font-bold">
                    ...
                    <div class="flex-1">
                        ...
                        @if(auth()->user()->bio)
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ auth()->user()->bio }}</p>
                        @endif
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Bergabung {{ auth()->user()->created_at->translatedFormat('d M Y') }}</p>
                    </div>
                    <a href="{{ route('profile.edit') }}" class="px-4 py-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg transition">
                        Edit Profil
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['simulations_played'] }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Dimainkan</div>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['following'] }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Mengikuti</div>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['followers'] }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Pengikut</div>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['comments'] }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Komentar</div>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $stats['unread_notifications'] }}</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-1">Notifikasi Baru</div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-4 hover:shadow-md transition">
                    ...
                    <div>
                        <div class="text-sm font-medium text-gray-900 dark:text-white">Edit Profil</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Ubah foto, nama, bio</div>
                    </div>
                </a>
                <a href="{{ route('notifications.index') }}" class="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-4 hover:shadow-md transition">
                    <div class="w-10 h-10 rounded-lg bg-orange-100 dark:bg-orange-900/50 text-orange-600 dark:text-orange-400 flex items-center justify-center">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                    </div>
                    <div>
                        <div class="text-sm font-medium text-gray-900 dark:text-white">Notifikasi</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $stats['unread_notifications'] }} belum dibaca</div>
                    </div>
                </a>
                <a href="{{ route('creators.show', auth()->user()->username) }}" class="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-4 hover:shadow-md transition">
                    <div class="w-10 h-10 rounded-lg bg-green-100 dark:bg-green-900/50 text-green-600 dark:text-green-400 flex items-center justify-center">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    </div>
                    <div>
                        <div class="text-sm font-medium text-gray-900 dark:text-white">Profil Publik</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400">Lihat profil Anda</div>
                    </div>
                </a>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm rounded-xl p-6">
                ...
                <div class="space-y-3">
                    @foreach($recent_bookmarks as $sim)
                        @if($sim)
                        <a href="{{ route('simulations.show', $sim->slug) }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            <div class="w-16 aspect-video bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden flex-shrink-0">
                                @if($sim->thumbnail)
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="text-sm font-medium text-gray-900 dark:text-white truncate">{{ $sim->title }}</h4>
                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ $sim->user->name }} &middot; {{ $sim->formatted_play_count }} dimainkan</p>
                            </div>
                        </a>
                        @endif
                    @endforeach
