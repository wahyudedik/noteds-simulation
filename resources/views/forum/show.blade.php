<x-app-layout>
    <x-slot name="header">
        <div class="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-3">
                <a href="{{ route('forum.index') }}" class="text-gray-400 dark:text-gray-500 hover:text-gray-600 transition">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        @if($thread->is_pinned)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400">
                                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M16 12V4h1V2H7v2h1v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z"/></svg>
                                Disematkan
                            </span>
                        @endif
                        @if($thread->is_solved)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-full bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400">
                        ...
                        @endif
                        @if($thread->is_locked)
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-full bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400">
                    ...
                    <h1 class="text-xl font-bold text-gray-900 dark:text-white truncate">{{ $thread->title }}</h1>
                </div>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="bg-white rounded-xl border border-gray-100 dark:bg-gray-800 dark:border-gray-700 p-6">
            <div class="flex items-start gap-4">
                ...
                <div x-data="{ votesCount: {{ $thread->votes_count }}, userVote: {{ $userVote ?? 'null' }} }"
                     @vote-updated.window="
                         if ($event.detail.votable_type === 'thread' && $event.detail.votable_id === {{ $thread->id }}) {
                             votesCount = $event.detail.votes_count;
                             userVote = $event.detail.user_vote;
                         }
                     "
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        @endif
                        <div>
                            <a href="{{ route('creators.show', $thread->user->username) }}" class="text-sm font-medium text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition">{{ $thread->user->name }}</a>
                            <p class="text-xs text-gray-400 dark:text-gray-500">{{ $thread->created_at->diffForHumans() }} · {{ number_format($thread->views_count) }} dilihat</p>
                        </div>
                    </div>

                    {{-- Body --}}
                    <div class="prose prose-sm max-w-none text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ $thread->body }}</div>

                    {{-- Simulation Link --}}
                    @if($thread->simulation)
                        <div class="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100 dark:bg-blue-900/20 dark:border-blue-800">
                            <div class="flex items-center gap-2">
                                ...
                                <a href="{{ route('simulations.show', $thread->simulation->slug) }}" class="text-sm font-medium text-blue-700 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition">
                    ...
                    @if(auth()->check() && auth()->user()->isAdmin())
                        <div class="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                            <form action="{{ route('forum.pin', $thread->slug) }}" method="POST">
                                @csrf
                                <button type="submit" class="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400 dark:hover:bg-gray-700 transition inline-flex items-center gap-1">
                    ...
                    @auth
                        @if($thread->isOwnedBy(auth()->user()))
                            <div class="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                                <a href="{{ route('forum.edit', $thread->slug) }}" class="text-xs px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400 dark:hover:bg-gray-700 transition inline-flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    Edit
                                </a>
                                <form action="{{ route('forum.destroy', $thread->slug) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmSubmit(this.closest('form'), 'Yakin ingin menghapus thread ini? Tindakan ini tidak dapat dibatalkan.', { title: 'Hapus Thread', confirmText: 'Ya, Hapus' })" class="text-xs px-3 py-1.5 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-900/30 transition inline-flex items-center gap-1">
        ...
        {{-- Replies --}}
        <div class="mt-6">
            <h2 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">{{ $thread->replies_count }} Balasan</h2>

            <div class="space-y-3">
                @forelse($replies->whereNull('parent_id') as $reply)
                    @include('forum._reply', ['reply' => $reply, 'thread' => $thread, 'depth' => 0])
                @empty
                    <div class="bg-white rounded-xl border border-gray-100 dark:bg-gray-800 dark:border-gray-700 p-8 text-center">
                        <p class="text-gray-400 dark:text-gray-500 text-sm">Belum ada balasan. Jadilah yang pertama membalas!</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        @auth
            @if(!$thread->is_locked)
                <div class="mt-6 bg-white rounded-xl border border-gray-100 dark:bg-gray-800 dark:border-gray-700 p-5">
                    <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">Tulis Balasan</h3>
                    <form action="{{ route('forum.reply', $thread->slug) }}" method="POST">
                        ...
                        <textarea name="body" rows="4" required
                                  ...
                                  class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-y dark:border-gray-600 dark:bg-gray-700 dark:text-white"></textarea>
            ...
            @else
                <div class="mt-6 bg-gray-50 rounded-xl border border-gray-200 dark:bg-gray-800 dark:border-gray-700 p-5 text-center">
        ...
        @else
            <div class="mt-6 bg-gray-50 rounded-xl border border-gray-200 dark:bg-gray-800 dark:border-gray-700 p-5 text-center">
                <a href="{{ route('login') }}" class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 text-sm font-medium">Masuk</a>
                <span class="text-gray-500 dark:text-gray-400 text-sm"> untuk membalas thread ini.</span>
            </div>
        @endauth
    </div>
    ...
    <script>
        function vote(votableType, votableId, value) {
            ...
            .then(data => {
                if (data.success) {
                    // Determine new userVote based on server action
                    let newUserVote = null;
                    if (data.action === 'added' || data.action === 'switched') {
                        newUserVote = value;
                    }
                    // 'removed' → newUserVote stays null

                    // Dispatch event for Alpine components to update state without page reload
