...
<div id="reply-{{ $reply->id }}" class="{{ $indentClass }} {{ $reply->is_accepted ? 'bg-green-50 border border-green-200 dark:bg-green-900/20 dark:border-green-800' : 'bg-white border border-gray-100 dark:bg-gray-800 dark:border-gray-700' }} rounded-xl p-4" x-data="{ showReplyForm: false, replyBody: '' }">
    <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-3 mt-2">
                @auth
                    @if(!$thread->is_locked)
                        <button @click="showReplyForm = !showReplyForm" class="text-xs text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 transition font-medium">
                            Balas
                        </button>
                    @endif
                    ...
                    @if($thread->isOwnedBy(auth()->user()) && !$reply->is_accepted)
                        <form action="{{ route('forum.reply.accept', $reply->id) }}" method="POST">
                            @csrf
                            <button type="submit" class="text-xs text-gray-400 dark:text-gray-500 hover:text-green-600 dark:hover:text-green-400 transition font-medium">
                ...
                <div x-show="showReplyForm" x-cloak class="mt-3">
                    <form action="{{ route('forum.reply', $thread->slug) }}" method="POST">
                        ...
                        <textarea name="body" rows="3" required x-model="replyBody"
                                  ...
                                  class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-y dark:border-gray-600 dark:bg-gray-700 dark:text-white"></textarea>
                        <div class="flex items-center gap-2 mt-2">
                            ...
                            <button type="button" @click="showReplyForm = false" class="px-4 py-1.5 text-xs text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition">
                                Batal
                            </button>
                        </div>
                    </form>
