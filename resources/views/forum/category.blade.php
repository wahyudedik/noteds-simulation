                ...
                <div class="space-y-3">
                    ...
                    @empty
                        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-12 text-center">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 font-medium">Belum ada thread di kategori ini.</p>
                    ...
                    <div class="space-y-1">
                        @foreach($categories as $cat)
                            <a href="{{ route('forum.category', $cat->slug) }}"
                               class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition group {{ $cat->id === $category->id ? 'bg-blue-50 dark:bg-blue-900/30' : 'hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                                <span class="w-3 h-3 rounded-full flex-shrink-0" style="background-color: {{ $cat->color ?? '#6B7280' }}"></span>
                                <span class="text-sm {{ $cat->id === $category->id ? 'text-blue-700 dark:text-blue-400 font-medium' : 'text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white' }} flex-1">{{ $cat->name }}</span>
