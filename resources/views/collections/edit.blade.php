<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">Edit Collection</h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Edit Form --}}
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6" x-data="{ saving: false }">
                <form action="{{ route('collections.update', $collection) }}" method="POST" @submit="saving = true">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="space-y-5">
                        ...
                        <div x-data="{ text: '{{ Str::replace("'", "\\'", old('description', $collection->description)) }}' }">
                            <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Deskripsi</label>
                            <textarea name="description" id="description" rows="3" x-model="text"
                    ...
                    <div class="flex items-center gap-3 mt-6">
                        <button type="submit" :disabled="saving" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white text-sm font-medium rounded-lg transition inline-flex items-center gap-2">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6" x-data="collectionSearch()">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Tambah Experience</h2>
                <div class="relative">
                    <input
                        type="text"
                        x-model="searchQuery"
                        ...
                        class="w-full pl-4 pr-10 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                    />
                    <svg class="absolute right-3 top-2.5 w-4 h-4 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                </div>
                ...
                <div x-show="results.length > 0" class="mt-3 space-y-2">
                    <template x-for="sim in results" :key="sim.id">
                        <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                            <div class="w-12 sm:w-16 aspect-video bg-gray-200 rounded overflow-hidden flex-shrink-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-900 dark:text-white line-clamp-1" x-text="sim.title"></p>
                                <p class="text-xs text-gray-500 dark:text-gray-400" x-text="sim.category"></p>
                            </div>
                            <button
                ...
                <p x-show="searchQuery.length >= 2 && results.length === 0 && !loading" class="mt-3 text-sm text-gray-400 dark:text-gray-500 text-center">
                    Tidak ada experience ditemukan
                </p>
            </div>

            {{-- Simulations in Collection --}}
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6" x-data="{ removing: null }">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Experience dalam Collection ({{ $collection->simulations->count() }})</h2>
                    ...
                    <div class="space-y-3">
                        @foreach($collection->simulations as $sim)
                            <div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                ...
                                <div class="flex-1 min-w-0">
                                    <a href="{{ route('simulations.show', $sim->slug) }}" class="text-sm font-medium text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition line-clamp-1">
                                        {{ $sim->title }}
                                    </a>
                                ...
                                <button
                                    ...
                                    class="p-2 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition"
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-500 dark:text-gray-400 text-sm">Belum ada experience dalam collection ini.</p>
                        <p class="text-gray-400 dark:text-gray-500 text-xs mt-1">Gunakan kolom pencarian di atas untuk menambahkan experience.</p>
                    </div>
                @endif
            </div>
        </div>
