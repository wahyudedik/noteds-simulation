        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100 dark:border-gray-700 mb-8">
                <div class="flex flex-col sm:flex-row items-start gap-6">
                    ...
                    {{-- Info --}}
                    <div class="flex-1">
                        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $collection->title }}</h1>
                        @if($collection->description)
                            <p class="text-gray-500 dark:text-gray-400 text-sm mt-2">{{ $collection->description }}</p>
                        @endif
                        ...
                        <div class="flex items-center gap-4 mt-4">
                            <a href="{{ route('creators.show', $collection->user->username) }}" class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-blue-600 transition">{{ $collection->user->name }}</span>
                            </a>
                        </div>
                        ...
                        @auth
                            @if(auth()->id() !== $collection->user_id)
                                <button
                                    ...
                                    class="mt-3 inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-full transition {{ $isSaved ? 'bg-blue-100 text-blue-700 hover:bg-blue-200 dark:bg-blue-900/50 dark:text-blue-400 dark:hover:bg-blue-900/70' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600' }}"
                                >
            ...
            {{-- Simulations --}}
            <div>
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Daftar Experience</h2>

                @if($collection->simulations->count() > 0)
                    <div class="space-y-3">
                        @foreach($collection->simulations as $index => $sim)
                            <a href="{{ route('simulations.show', $sim->slug) }}" class="flex items-center gap-4 bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition group">
                                <div class="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 flex items-center justify-center text-sm font-semibold flex-shrink-0">
                                    {{ $index + 1 }}
                                </div>
                                <div class="w-32 aspect-video bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden flex-shrink-0">
                                    @if($sim->thumbnail)
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-sm font-medium text-gray-900 dark:text-white group-hover:text-blue-600 transition line-clamp-1">{{ $sim->title }}</h3>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $sim->user->name }} &middot; {{ $sim->formatted_play_count }} dimainkan</p>
                                </div>
                                <svg class="w-5 h-5 text-gray-400 dark:text-gray-500 group-hover:text-blue-600 transition flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                            </a>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-16">
                        ...
                        <p class="text-gray-400 dark:text-gray-500 text-sm mt-2">Collection ini belum memiliki experience.</p>
                    </div>
                @endif
            </div>
        </div>
