        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs text-gray-500 dark:text-gray-400">Total Iklan</p>
                    <p class="text-xl font-bold text-gray-900 dark:text-white mt-1">{{ $stats['total_ads'] }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs text-gray-500 dark:text-gray-400">Aktif</p>
                    <p class="text-xl font-bold text-green-600 dark:text-green-400 mt-1">{{ $stats['active_ads'] }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs text-gray-500 dark:text-gray-400">Impressions</p>
                    <p class="text-xl font-bold text-gray-900 dark:text-white mt-1">{{ number_format($stats['total_impressions']) }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs text-gray-500 dark:text-gray-400">Klik</p>
                    <p class="text-xl font-bold text-gray-900 dark:text-white mt-1">{{ number_format($stats['total_clicks']) }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs text-gray-500 dark:text-gray-400">CTR</p>
                    <p class="text-xl font-bold text-blue-600 dark:text-blue-400 mt-1">{{ $stats['ctr'] }}%</p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-xs text-gray-500 dark:text-gray-400">Revenue</p>
                    <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">Rp {{ number_format($stats['total_revenue'], 0, ',', '.') }}</p>
                </div>
            </div>

            {{-- Filters --}}
            <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm mb-6">
                <form method="GET" action="{{ route('admin.ads.index') }}" class="flex flex-wrap items-end gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="active_only" value="1" {{ request('active_only') ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 dark:text-blue-400">
                        <label class="text-sm text-gray-600 dark:text-gray-400">Hanya Aktif</label>
                    </div>
                ...
                <div class="p-6">
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b border-gray-200 dark:border-gray-700">
                                        ...
                                        <th class="text-left py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Tipe</th>
                                        <th class="text-left py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Posisi</th>
                                        <th class="text-center py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Impressions</th>
                                        <th class="text-center py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Klik</th>
                                        <th class="text-center py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">CTR</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b border-gray-100 hover:bg-gray-50 dark:bg-gray-700/50 dark:hover:bg-gray-600/50">
                                        ...
                                        <td class="py-3 px-2 text-center text-gray-500 dark:text-gray-400">{{ number_format($ad->clicks) }}</td>
                                        <td class="py-3 px-2 text-center text-gray-500 dark:text-gray-400">{{ $ad->ctr }}%</td>
                                        ...
                                        <td class="py-3 px-2 text-right space-x-2">
                                            <a href="{{ route('admin.ads.edit', $ad) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 text-xs font-medium">Edit</a>
                                            <form action="{{ route('admin.ads.toggle', $ad) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-700 text-xs font-medium">
                                            ...
                                            <form action="{{ route('admin.ads.destroy', $ad) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmSubmit(this.closest('form'), 'Yakin hapus iklan ini?')" class="text-red-600 dark:text-red-400 hover:text-red-700 text-xs font-medium">Hapus</button>
                                            </form>
                                        </td>
                                    </tr>
                                    @endforeach
                    ...
                    @else
                        <div class="text-center py-8 text-gray-500 dark:text-gray-400">
                            ...
                            <a href="{{ route('admin.ads.create') }}" class="mt-2 inline-flex items-center gap-1 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 text-sm font-medium">
