<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">{{ $challenge->title }}</h2>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Detail</h3>
                        <dl class="space-y-1 text-sm">
                            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Tipe</dt><dd class="font-medium">{{ $challenge->type_label }}</dd></div>
                            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Tema</dt><dd class="font-medium">{{ $challenge->theme }}</dd></div>
                            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Peserta</dt><dd class="font-medium">{{ $entries->count() }}</dd></div>
                        </dl>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Periode</h3>
                        <dl class="space-y-1 text-sm">
                            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Mulai</dt><dd class="font-medium">{{ $challenge->start_date->format('d M Y H:i') }}</dd></div>
                            <div class="flex justify-between"><dt class="text-gray-500 dark:text-gray-400">Selesai</dt><dd class="font-medium">{{ $challenge->end_date->format('d M Y H:i') }}</dd></div>
                        </dl>
                    </div>
                    <div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Peserta ({{ $entries->count() }})</h3>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Skor</th>
                                ...
                                <tr class="hover:bg-gray-50 dark:bg-gray-700/50 dark:hover:bg-gray-600/50">
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        @if ($challenge->status === 'judging' || $challenge->status === 'active')
                                            <button onclick="document.getElementById('score-modal-{{ $entry->id }}').classList.remove('hidden')" class="text-blue-600 dark:text-blue-400 hover:text-blue-800">Beri Skor</button>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-gray-400">Belum ada peserta.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 p-6">
                ...
                <form method="POST" action="{{ route('admin.challenges.score-entry', ['challenge' => $challenge, 'entry' => $entry]) }}">
                    ...
                    <div class="flex justify-end gap-3 mt-4">
                        <button type="button" onclick="document.getElementById('score-modal-{{ $entry->id }}').classList.add('hidden')" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 hover:bg-gray-200 rounded-lg">Batal</button>
