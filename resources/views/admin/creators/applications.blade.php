<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.creators.index') }}" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg transition">
                ? Kembali
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex gap-2 mb-6">
                <a href="{{ route('admin.creators.applications', ['status' => 'pending']) }}"
                   class="px-4 py-2 text-sm font-medium rounded-lg transition {{ request('status', 'pending') === 'pending' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' : 'bg-white text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:bg-gray-700/50 dark:hover:bg-gray-600/50 border border-gray-200' }}">
                    Menunggu ({{ $counts['pending'] }})
                </a>
                <a href="{{ route('admin.creators.applications', ['status' => 'approved']) }}"
                   class="px-4 py-2 text-sm font-medium rounded-lg transition {{ request('status') === 'approved' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-white text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:bg-gray-700/50 dark:hover:bg-gray-600/50 border border-gray-200' }}">
                    Disetujui ({{ $counts['approved'] }})
                </a>
                <a href="{{ route('admin.creators.applications', ['status' => 'rejected']) }}"
                   class="px-4 py-2 text-sm font-medium rounded-lg transition {{ request('status') === 'rejected' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-white text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:bg-gray-700/50 dark:hover:bg-gray-600/50 border border-gray-200' }}">
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div>
                                        <div class="flex items-center gap-2">
                                            <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ $application->user->name }}</h3>
                                        ...
                                        <p class="text-sm text-gray-700 dark:text-gray-300 mt-2">{{ $application->reason }}</p>
                                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">Diajukan {{ $application->created_at->diffForHumans() }}</p>
                                        @if($application->reviewed_at)
                                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Direview {{ $application->reviewed_at->diffForHumans() }} oleh {{ $application->reviewer?->name }}</p>
                                        @endif
                                        @if($application->review_notes)
                                            <p class="text-xs text-gray-500 dark:text-gray-400 italic mt-1">"{{ $application->review_notes }}"</p>
                                        @endif
                                    </div>
                                </div>
                                    ...
                                    <div class="flex gap-2 shrink-0">
                                        ...
                                        <form action="{{ route('admin.creators.applications.reject', $application) }}" method="POST" class="inline" x-data="{ showReject: false }">
                                            ...
                                            <div x-show="showReject" x-cloak class="flex gap-2">
                                                ...
                                                <button type="button" @click="showReject = false" class="px-3 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg transition">
                                                    Batal
                                                </button>
                                            </div>
                                        </form>
            ...
            @else
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-12 shadow-sm text-center">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">Belum ada pengajuan creator dengan status ini.</p>
                </div>
            @endif
        </div>
    </div>
