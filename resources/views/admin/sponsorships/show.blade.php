        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <div class="flex items-start justify-between mb-4">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $sponsorship->title }}</h3>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                    Sponsor: <a href="{{ route('admin.sponsors.show', $sponsorship->sponsor) }}" class="text-blue-600 dark:text-blue-400 hover:underline">{{ $sponsorship->sponsor->company_name }}</a>
                                </p>
                            </div>
                        ...
                        <div class="flex flex-wrap gap-2 mb-4">
                            ...
                            @endif
                            <a href="{{ route('admin.sponsorships.edit', $sponsorship) }}" class="px-3 py-1.5 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 hover:bg-gray-200 rounded-lg transition">Edit</a>
                            <a href="{{ route('admin.sponsorships.invoices', $sponsorship) }}" class="px-3 py-1.5 text-xs font-medium text-purple-700 bg-purple-100 dark:bg-purple-900/30 hover:bg-purple-200 rounded-lg transition inline-flex items-center gap-1">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100">
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Paket</p>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $sponsorship->package_label }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Budget</p>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">Rp {{ number_format($sponsorship->budget, 0, ',', '.') }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Terkonsumsi</p>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">Rp {{ number_format($sponsorship->spent, 0, ',', '.') }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Sisa Budget</p>
                                <p class="text-sm font-medium {{ $sponsorship->remaining_budget > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">Rp {{ number_format($sponsorship->remaining_budget, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 text-center">
                            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['total_impressions'] ?? $sponsorship->total_impressions) }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Impressions</p>
                        </div>
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 text-center">
                            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['total_clicks'] ?? $sponsorship->total_clicks) }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Clicks</p>
                        </div>
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 text-center">
                            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['ctr'] ?? $sponsorship->ctr, 2) }}%</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">CTR</p>
                        </div>
                    </div>

                    {{-- Linked Ads --}}
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">Iklan Terkait ({{ $sponsorship->platformAds->count() }})</h3>
                        @if($sponsorship->platformAds->count() > 0)
                            <div class="space-y-3">
                                @foreach($sponsorship->platformAds as $ad)
                                    <div class="flex items-center justify-between p-3 rounded-lg border border-gray-100 hover:bg-gray-50 dark:bg-gray-700/50 dark:hover:bg-gray-600/50">
                                        <div class="flex items-center gap-3">
                                            @if($ad->image_path)
                                                <img src="{{ asset('storage/' . $ad->image_path) }}" alt="{{ $ad->title }}" class="w-10 h-10 rounded-lg object-cover">
                                            @else
                                                <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-400 dark:text-gray-500 text-xs">IMG</div>
                                            @endif
                                            <div>
                        ...
                        @else
                            <div class="text-center py-6 text-gray-500 dark:text-gray-400 text-sm">
                                ...
                                <p class="text-xs mt-1">Buat iklan melalui <a href="{{ route('admin.ads.index') }}" class="text-blue-600 dark:text-blue-400 hover:underline">Manajemen Iklan</a> dan hubungkan ke sponsorship ini.</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                                ...
                                @foreach($invoices->take(3) as $invoice)
                                    <div class="flex items-center justify-between p-3 rounded-lg border border-gray-100">
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500 dark:text-gray-400">Jatuh tempo: {{ $invoice->due_date->format('d M Y') }}</p>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-sm font-medium text-gray-900 dark:text-white">Rp {{ number_format($invoice->amount, 0, ',', '.') }}</p>
                        ...
                        @else
                            <div class="text-center py-4 text-gray-500 dark:text-gray-400 text-sm">Belum ada invoice.</div>
                        @endif
                    </div>
                </div>
                ...
                <div class="lg:col-span-1 space-y-6">
                    {{-- Positions --}}
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">Posisi Iklan</h3>
                        <div class="flex flex-wrap gap-2">
                            @if($sponsorship->positions)
                                @foreach($sponsorship->positions as $pos)
                                    <span class="inline-flex px-2.5 py-1 text-xs font-medium bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 rounded-full">{{ ucfirst(str_replace('_', ' ', $pos)) }}</span>
                                @endforeach
                            @else
                                <span class="text-xs text-gray-500 dark:text-gray-400">Semua posisi</span>
                            @endif
                        </div>
                    </div>
                    ...
                    {{-- Schedule --}}
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">Jadwal</h3>
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-gray-400">Mulai</span>
                                <span class="text-gray-900 dark:text-white">{{ $sponsorship->start_date->format('d M Y') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-gray-400">Selesai</span>
                                <span class="text-gray-900 dark:text-white">{{ $sponsorship->end_date->format('d M Y') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-gray-400">Durasi</span>
                                <span class="text-gray-900 dark:text-white">{{ $sponsorship->start_date->diffInDays($sponsorship->end_date) }} hari</span>
                            </div>
                    ...
                    {{-- Category Filter --}}
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">Filter Kategori</h3>
                        @if($sponsorship->category_filter && count($sponsorship->category_filter) > 0)
                            <div class="flex flex-wrap gap-2">
                                @foreach($sponsorship->category_filter as $cat)
                                    <span class="inline-flex px-2.5 py-1 text-xs font-medium bg-gray-100 text-gray-700 dark:text-gray-300 rounded-full">{{ $cat }}</span>
                                @endforeach
                            </div>
                        @else
                            <p class="text-xs text-gray-500 dark:text-gray-400">Semua kategori</p>
                        @endif
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="text-gray-900 dark:text-white">{{ $sponsorship->created_at->format('d M Y H:i') }}</span>
                            </div>
                            @if($sponsorship->approved_at)
                                <div class="flex justify-between">
                                    <span class="text-gray-500 dark:text-gray-400">Disetujui</span>
                                    <span class="text-gray-900 dark:text-white">{{ $sponsorship->approved_at->format('d M Y H:i') }}</span>
                                </div>
                            @endif
                            @if($sponsorship->target_impressions)
                                <div class="flex justify-between">
                                    <span class="text-gray-500 dark:text-gray-400">Target Imp</span>
                                    <span class="text-gray-900 dark:text-white">{{ number_format($sponsorship->target_impressions) }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
