    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">Buat Invoice Baru</h3>
                ...
                <div class="p-6">
                    @if($invoices->count() > 0)
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b border-gray-200 dark:border-gray-700">
                                        <th class="text-left py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Nomor Invoice</th>
                                        <th class="text-right py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Jumlah</th>
                                        <th class="text-center py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Jatuh Tempo</th>
                                        <th class="text-center py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Status</th>
                                        <th class="text-center py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Dibayar</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b border-gray-100 hover:bg-gray-50 dark:bg-gray-700/50 dark:hover:bg-gray-600/50">
                                        <td class="py-3 px-2 font-medium text-gray-900 dark:text-white">{{ $invoice->invoice_number }}</td>
                                        <td class="py-3 px-2 text-right text-gray-900 dark:text-white font-medium">Rp {{ number_format($invoice->amount, 0, ',', '.') }}</td>
                                        <td class="py-3 px-2 text-center text-gray-500 dark:text-gray-400">{{ $invoice->due_date->format('d M Y') }}</td>
                                        <td class="py-3 px-2 text-center">
                                            <span class="inline-flex px-2 py-0.5 text-xs font-medium {{ $invoice->status_color }} rounded-full">{{ $invoice->status_label }}</span>
                                        </td>
                                        <td class="py-3 px-2 text-center text-gray-500 dark:text-gray-400 text-xs">
                                            @if($invoice->paid_at)
                                                {{ $invoice->paid_at->format('d M Y') }}<br>
                                                <span class="text-gray-400 dark:text-gray-500">{{ $invoice->payment_method }}</span>
                                            @else
                                                -
                                            @endif
                                        </td>
                                        <td class="py-3 px-2 text-right space-x-2">
                                            @if($invoice->status === 'draft')
                                                <form action="{{ route('admin.invoices.send', $invoice) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 text-xs font-medium">Kirim</button>
                                                </form>
                                            @endif
                                            @if(in_array($invoice->status, ['draft', 'sent', 'overdue']))
                                                <form action="{{ route('admin.invoices.mark-paid', $invoice) }}" method="POST" class="inline" x-data="{ show: false }">
                                                    ...
                                                    <button type="submit" class="text-green-600 dark:text-green-400 hover:text-green-700 text-xs font-medium">Bayar</button>
                                                </form>
                                            @endif
                                        </td>
                                    </tr>
                        ...
                        <div class="mt-6 pt-4 border-t border-gray-100 grid grid-cols-3 gap-4">
                            ...
                            <div class="text-center">
                                <p class="text-lg font-bold text-green-600 dark:text-green-400">Rp {{ number_format($invoices->where('status', 'paid')->sum('amount'), 0, ',', '.') }}</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Sudah Dibayar</p>
                            </div>
                            <div class="text-center">
                                <p class="text-lg font-bold text-amber-600 dark:text-amber-400">Rp {{ number_format($invoices->where('status', '!=', 'paid')->sum('amount'), 0, ',', '.') }}</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">Belum Dibayar</p>
                            </div>
                        </div>
                    @else
