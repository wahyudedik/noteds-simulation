                    ...
                    <form action="{{ route('admin.simulations.update', $simulation) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">{{ old('description', $simulation->description) }}</textarea>
                        </div>

                        {{-- Thumbnail --}}
                        <div>
                            <label for="thumbnail" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Thumbnail Baru</label>
                            @if($simulation->thumbnail)
                                <div class="mb-2">
                                    <img src="{{ Storage::disk('public')->url($simulation->thumbnail) }}" alt="Thumbnail" class="w-32 h-20 object-cover rounded-lg" />
                                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Thumbnail saat ini</p>
                                </div>
                            @endif
                        ...
                        <div class="flex items-center gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <a href="{{ route('admin.simulations.show', $simulation) }}" class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg transition">
                                Batal
                            </a>
                        </div>
                    </form>
