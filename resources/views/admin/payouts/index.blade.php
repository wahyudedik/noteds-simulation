        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    ...
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Rp {{ number_format($stats['total_pending'], 0, ',', '.') }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Disetujui</p>
                    <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">Rp {{ number_format($stats['total_approved'], 0, ',', '.') }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Sudah Dibayar</p>
                    <p class="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">Rp {{ number_format($stats['total_paid'], 0, ',', '.') }}</p>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ $stats['paid_count'] }} transaksi</p>
                </div>
                <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-4 shadow-sm">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Total Creator</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ $stats['total_creators'] }}</p>
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                ...
                <form method="GET" action="{{ route('admin.payouts.index') }}" class="flex items-center gap-2">
                    ...
                    <button type="submit" class="px-3 py-1.5 text-sm bg-gray-100 text-gray-600 dark:text-gray-400 rounded-lg hover:bg-gray-200">Cari</button>
                </form>
            </div>
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b border-gray-200 dark:border-gray-700">
                                        ...
                                        <th class="text-left py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Rekening</th>
                                        <th class="text-center py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Status</th>
                                        <th class="text-left py-3 px-2 text-gray-500 dark:text-gray-400 font-medium">Diajukan</th>
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-b border-gray-100 hover:bg-gray-50 dark:bg-gray-700/50 dark:hover:bg-gray-600/50">
                                        <td class="py-3 px-2">
                                            ...
                                            <div class="text-xs text-gray-400 dark:text-gray-500">{{ $payout->user->email }}</div>
                                        </td>
                                        <td class="py-3 px-2 text-right">
                                            <span class="font-semibold text-gray-900 dark:text-white">{{ $payout->formatted_amount }}</span>
                                            @if($payout->amount_usd)
                                                <div class="text-xs text-gray-400 dark:text-gray-500">${{ number_format($payout->amount_usd, 2) }}</div>
                                            @endif
                                        </td>
                                        <td class="py-3 px-2">
                                            <span class="inline-flex px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-600 dark:text-gray-400 rounded-full">
                                        ...
                                        <td class="py-3 px-2 text-right">
                                            <a href="{{ route('admin.payouts.show', $payout) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 text-xs font-medium">Detail</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
