    ...
    @if($listing)
        <div class="bg-white dark:bg-gray-800 border border-red-100 dark:border-red-900/50 rounded-xl p-6 shadow-sm">
            ...
            <form action="{{ route('studio.simulations.marketplace.remove', $simulation->slug) }}" method="POST"
                ...
                x-on:submit.prevent="showConfirm('Menghapus listing akan menonaktifkan penjualan simulasi ini.', { title: 'Hapus dari Marketplace', confirmText: 'Ya, Hapus' }).then(confirmed => { if (confirmed) { $el.submit(); } })">
                @csrf
                @method('DELETE')
