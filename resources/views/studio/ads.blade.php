<x-studio-layout :pageTitle="'Iklan Experience'">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Iklan: {{ $simulation->title }}</h2>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-6 mb-6 shadow-sm">
        <h3 class="font-semibold text-gray-900 dark:text-white mb-4">Tambah Iklan Baru</h3>

        @if($errors->any())
            <div class="bg-red-50 border border-red-200 text-red-700 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400 text-sm rounded-lg p-3 mb-4">
        ...
        <form action="{{ route('studio.simulations.ads.store', $simulation->slug) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                {{-- Provider --}}
                <div>
                    <label for="provider" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Provider Iklan</label>
                    <select name="provider" id="provider" x-data x-model="$el.value" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                {{-- Publisher ID --}}
                <div>
                    <label for="publisher_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Publisher / Ad Slot ID</label>
                    <input type="text" name="publisher_id" id="publisher_id" value="{{ old('publisher_id') }}"
                           class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500"
                           placeholder="ca-pub-1234567890" required />
                </div>

                {{-- Ad Type --}}
                <div>
                    <label for="ad_type" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tipe Iklan</label>
                    <select name="ad_type" id="ad_type" x-data x-model="$el.value" class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                {{-- Title --}}
                <div>
                    <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Judul (opsional)</label>
                ...
                {{-- Image URL --}}
                <div>
                    <label for="image_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">URL Gambar (opsional)</label>
                ...
                {{-- Target URL --}}
                <div>
                    <label for="target_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">URL Tujuan (opsional)</label>
                ...
                {{-- Code Snippet --}}
                <div class="md:col-span-2">
                    <label for="code_snippet" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Code Snippet (opsional)</label>
                    <textarea name="code_snippet" id="code_snippet" rows="4"
                              class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-lg text-sm font-mono focus:border-blue-500 focus:ring-blue-500"
                              placeholder="Kode HTML/JS iklan...">{{ old('code_snippet') }}</textarea>
                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Kode akan di-scan oleh sistem keamanan sebelum disetujui.</p>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm">
        <div class="p-6 border-b border-gray-100 dark:border-gray-700">
            <h3 class="font-semibold text-gray-900 dark:text-white">Iklan yang Sudah Diajukan</h3>
        </div>
                ...
                <table class="min-w-[600px] w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50">
                            <th class="text-left py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">Provider</th>
                            <th class="text-left py-3 px-4 text-gray-500 dark:text-gray-400 font-medium">Publisher ID</th>
                    ...
                    <tbody>
                        @foreach($creatorAds as $ad)
                            <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                <td class="py-3 px-4 text-gray-900 dark:text-white">{{ ucfirst($ad->provider) }}</td>
                                <td class="py-3 px-4 text-gray-500 dark:text-gray-400 font-mono text-xs">{{ $ad->publisher_id }}</td>
                                <td class="py-3 px-4 text-gray-500 dark:text-gray-400">{{ ucfirst($ad->ad_config['type'] ?? '-') }}</td>
