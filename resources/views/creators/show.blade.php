<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">{{ $creator->name }}</h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100 dark:border-gray-700">
                <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                    ...
                    <div class="flex-1 text-center sm:text-left">
                        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $creator->name }}
                            @if($creator->role === 'superadmin')
                                <span class="inline-flex items-center ml-2 px-2 py-0.5 text-xs font-medium rounded-full bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-400">
                            ...
                            @elseif($creator->role === 'admin')
                                <span class="inline-flex items-center ml-2 px-2 py-0.5 text-xs font-medium rounded-full bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-400">
                                    <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                                    Admin
                                </span>
                            @elseif($creator->role === 'creator')
                                <span class="inline-flex items-center ml-2 px-2 py-0.5 text-xs font-medium rounded-full bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400">
                            ...
                            @if($isTop10)
                                <span class="inline-flex items-center ml-2 px-2 py-0.5 text-xs font-medium rounded-full bg-yellow-100 dark:bg-yellow-900/50 text-yellow-700 dark:text-yellow-400">
                                    <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                                    Top {{ $rankingPosition }} Creator
                                </span>
                            @endif
                        ...
                        @if($creator->bio)
                            <p class="text-gray-500 dark:text-gray-400 text-sm mt-2 max-w-lg">{{ $creator->bio }}</p>
                        @endif
                        <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">Bergabung {{ $creator->created_at->translatedFormat('d M Y') }}</p>

                        <div class="flex items-center gap-6 mt-4 justify-center sm:justify-start">
                            <div class="text-center">
                                <p class="text-gray-900 dark:text-white font-bold text-lg">{{ $creator->published_simulations_count }}</p>
                                <p class="text-gray-500 dark:text-gray-400 text-xs">Experience</p>
                            </div>
                            <div class="text-center">
                                <p class="text-gray-900 dark:text-white font-bold text-lg">{{ $creator->followers_count }}</p>
                                <p class="text-gray-500 dark:text-gray-400 text-xs">Pengikut</p>
                            </div>
                            <div class="text-center">
                                <p class="text-gray-900 dark:text-white font-bold text-lg">{{ $creator->following_count }}</p>
                        ...
                        <div class="mt-4 flex gap-3">
                            @auth
                                @if(auth()->id() !== $creator->id)
                                    <button
                                        ...
                                        class="px-6 py-2 text-sm font-medium rounded-full transition {{ $isFollowing ? 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600' : 'bg-blue-600 text-white hover:bg-blue-700' }}"
                                    >
                            ...
                            @endauth
                            <button onclick="copyProfileLink()" class="px-4 py-2 text-sm font-medium rounded-full bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 transition">
            ...
            <div class="mt-8">
                        ...
                        @foreach($simulations as $sim)
                            <a href="{{ route('simulations.show', $sim->slug) }}" class="simulation-card group">
                                <div class="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700">
                                    ...
                                    <div class="p-3">
                                        <h3 class="text-gray-900 dark:text-white text-sm font-medium line-clamp-2 group-hover:text-blue-600 transition">{{ $sim->title }}</h3>
                ...
                @else
                    <div class="text-center py-16">
                        ...
                        <h3 class="text-gray-500 dark:text-gray-400 text-lg font-medium">Belum ada experience</h3>
                        <p class="text-gray-400 dark:text-gray-500 text-sm mt-2">Creator ini belum mengunggah experience apapun.</p>
                    </div>
                @endif
            </div>
        </div>
